...
dojo.declare("wm.DraggableTreeMover", wm.DragDropper, {
    ...
    drag: function(e) {
        ...
        var r = {
            l: this.pxp - this.rootOffset.x,
            t: this.pyp - this.rootOffset.y,
            w: 0,
            h: 0
        };
        ...
        if (!this.target) {
            ...
            if (!this.manager.dropBetweenNodes) {
                ...
                case "top":
                    ...
                    break;
                case "bot":
                    if (!this.target.closed && !this.manager.getNoDrop(this.target)) if (this.target.declaredClass == "wm.TreeNode" && this.target.kids.length == 0) {
                        caption += " after ";
                    } else {
                        caption += " first child of ";
                    } else if (isParentDropTarget) caption += " after ";
                    else {
                        this.setAvatarContent("Moving <b>" + this.info.caption + "</b>");
                        return;
                    }
    ...
    findTarget: function(inHit) {
        ...
        if (t == this.manager.draggedItem) t = null;
        if (t != this.target) {
            //console.log("SET TARGET TO " + ((t==null) ? "NULL" : t.data.getItemName()));
            this.setTarget(t);
            if (this.target) this.setTargetArea(this._targetArea);
        } else if (this.target && this.targetArea != this._targetArea) this.setTargetArea(this._targetArea);
        this.updateAvatar();
    },
    _findTarget: function(inHit) {
        ...
        if (y <= onethird) this._targetArea = "top";
        else if (y <= onethird * 2) this._targetArea = "mid";
        else this._targetArea = "bot";
