...
dojo.declare("wm.PropertyTree", wm.Tree, {
    ...
    setDataSet: function(inDataSet) {
        ...
        if (inDataSet) this.selectedItem.setType(inDataSet.type);
        this.buildTree();
    },
    set_dataSet: function(inDataSet) {
        ...
        if (inDataSet && !(inDataSet instanceof wm.Variable)) {
            ...
            if (ds) this.components.binding.addWire("", "dataSet", ds.getId());
        } else this.setDataSet(inDataSet);
    },
    clear: function() {
        this.inherited(arguments);
    ...
    buildTree: function() {
        ...
        for (var i = 0; i < size; i++) {
            ...
            var node = new wm.TreeNode(this.root, {
                closed: true,
                data: item,
                dataValue: null,
            ...
            if (hasChild) {
                var blankChild = new wm.TreeNode(node, {
                    close: true,
            ...
            if (value instanceof wm.Variable) {
                ...
                if (variable.isList) {
                    ...
                    for (var i = 0; i < size; i++) {
                        ...
                        var node = new wm.TreeNode(inParentNode, {
                            closed: true,
                            data: item,
                            propertyName: prop,
                            dataValue: null,
    ...
    select: function(inNode) {
        if (this.selected != inNode) {
            ...
            while (node != this.root) {
                if (dojo.indexOf(data, node.data) == -1) data.push(node.data);
                node = node.parent;
            }
