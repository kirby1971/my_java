...
/*
 * DojoTreeGrid adds the treeGridGroupByField which creates a tree grouped by the specified field; not deployed because of 
 * amount of spagetti code needed for this to work.
 * 1. Should be a subclass of DojoGrid
 * 2. Editing fields should work
 * 3. Should be able to use LiveViews and lazy loading just like PropertyTree to load subobjects of a different type
 */
...
dojo.declare("wm.DojoGrid", wm.Control, {
    ...
    treeGridGroupByField: "",
    treeGridDefaultOpen: true,
	...
	*/
	...
	renderDojoObj: function() {
	  if (this._cupdating)
	    ...
	    if (!this.treeGridGroupByField) {
		this.dojoObj = new dojox.grid.DataGrid(props,dojo.create('div', {style:'width:100%;height:100%'}, this.gridNode));
	    } else {
		props.queryOptions = {deep:true};
		props.rowSelector = false; // adds selection area/margin to the left
		props.openAtLevels = [true];
		props.defaultOpen = this.treeGridDefaultOpen;
		dojo["require"]("dojox.grid.TreeGrid");
		dojo["require"]("dijit.tree.ForestStoreModel");
		props.treeModel = new dijit.tree.ForestStoreModel({
                        deferItemLoadingUntilExpand:true,

		    identifier: this.treeGridGroupByField,
		    store: this.store,
		    query: this.query || {_root: "root"},
		    rootLabel: "state",
		...
		this.dojoObj = new dojox.grid.TreeGrid(props,dojo.create('div', {style:'width:100%;height:100%'}, this.gridNode));
	    }
...
/*
	...
	/* Feeding in the data for each call to this is unnecessary overhead; users need to call getRowData(inRowIndex) if they are going to use this */
        onRenderRow: function(inRowIndex, inRowNode) {},
	getDataSet: function() {
	...
	setDojoStore: function(){
		...
		if (!this.treeGridGroupByField) {
		    ...
		    var index = {};
		    for (var i = 0; i < dataList.length; i++) {
			var item = this.variable.getItem(i);
			dataList[i]._wmVariable = item;
			var data = wm.flattenObject(item.getData());
			data._wmVariable = item;

			var groupByValue;
			if (data[this.treeGridGroupByField] !== undefined) {
			    groupByValue = data[this.treeGridGroupByField];
			} else {
			    var subitem = item.getValue(this.treeGridGroupByField);
			    if (subitem) {
				groupByValue = dojo.toJson(subitem.getData());
			    }
			    if (this.columns[0].field != "_subobject") {
				this.columns.unshift({show: true,
						      field: "_subobject",
						      title: "",
						      width: "20px"});
			    }
			...
			if (!index[groupByValue]) {
			    var obj;
			    if (subitem) {
				obj = subitem.getData();
				obj._wmVariable = subitem;
				obj.children = [];
				obj._type = subitem.type;
			    } else {
				obj = {children: []};
			    }
			    ...
			    obj._root = "root";
			    if (!subitem)
				obj[this.treeGridGroupByField] =  groupByValue;
			    for (var j = 0; j < this.columns.length; j++) {
				    var id = this.columns[j].field;
				    var indexOfId = id.indexOf(this.treeGridGroupByField + ".");
				    if (indexOfId == 0)
					id = id.substring((this.treeGridGroupByField + ".").length);
				    
				    if (obj[id] === undefined)
					obj[id] = "";
				}
			    index[groupByValue] = obj;
			    newDataList.push(obj);
			}
			if (!subitem)
			    delete data[this.treeGridGroupByField];
			index[groupByValue].children.push(data);
		    }
	...
	getStructure: function(){
		...
		dojo.forEach(this.columns, function(col){
		    ...
		    if (obj.field == "_subobject") {
			    obj.get = function() {return "";}
		    }
...
/*
		    if (this.treeGridGroupByField == obj.field) {
			treeGridColumn = obj;
		    } else {
		    */
			structure.push(obj);
//		    }
    ...
    updateColumnData: function () {
        ...
        dojo.forEach(viewFields, function (f, i) {
            ...
            if (wm.typeManager.isPropInList(wm.typeManager.getType(this.variable.type).fields, f.dataIndex)) return;
	...
	*/
	...
	_onGridEvent: function(evt){
		...
		if (selectedData)
			this.selectItemOnGrid(this.selectedItem);
	    if (this.treeGridGroupByField) {
		var structure = this.dojoObj.structure[0][0].cells;
	    } else {
...
*/
	...
	getExpressionValue: function(exp, idx, dataObj, isSimpleDataObj){
		...
		try
		{
		    if (this.treeGridGroupByField && json._type) {
			var r = new RegExp("\\$\\{" + this.treeGridGroupByField + "\\.", "g");
			exp = exp.replace(r, "\${");
		    }
    ...
    customFormatter: function(formatFunc, backgroundColorFunc, textColorFunc,cssClassFunc, inValue, rowIdx, cellObj, treeCellObj){
	var cellObj = dojox.grid.TreeGrid && this.dojoObj instanceof dojox.grid.TreeGrid ? treeCellObj : cellObj;
