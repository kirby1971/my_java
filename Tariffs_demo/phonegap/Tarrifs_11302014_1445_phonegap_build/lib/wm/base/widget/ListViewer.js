...
dojo.declare("wm.ListViewerRow", wm.Container, {
    ...
    */
    ...
    init: function() {
        ...
        this.variable = new wm.Variable({
            name: "variable",
            owner: this
        });
        this.itemNumber = new wm.Variable({
            name: "itemNumber",
            owner: this,
            type: "NumberData"
        });
        ...
        if (!this.variable.$.binding) new wm.Binding({
            name: "binding",
            owner: this.variable
        });
...
dojo.declare("wm.ListViewer", wm.Container, {
    ...
    init: function() {
        ...
        if (this.dataSet && this.dataSet._requester) this.setLoadingImageShowing(true);
    ...
    setBorderColor: function(inColor) {
        this.inherited(arguments);
        dojo.forEach(this.rowRenderers, function(r) {
            r.setBorderColor(inColor);
        });
    },
    setRowBorder: function(inBorder) {
        this.rowBorder = inBorder;
        dojo.forEach(this.rowRenderers, function(r) {
            r.setBorder(inBorder);
        });
    },
    ...
    setPageName: function(inPage) {
        ...
        if (!inPage) {
            ...
            dojo.forEach(this.rowRenderers[i].c$, function(e) {
                e.destroy();
            });
            return;
        }
        ...
        try {
            if (!dojo.getObject(inPage)) window[inPage] = {};
    ...
    cleanupWidgets: function(inComponents) {
        for (var i in inComponents) {
            ...
            if (c[0] == "wm.Layout") {
                c[0] = "wm.Panel";
                if (!c[1].layoutKind) c[1].layoutKind = "top-to-bottom"; // layout kind that is default for wm.Layout
                c[1].height = "100px";
    ...
    setDataSet: function(inDataSet) {
        ...
        if (inDataSet && inDataSet instanceof wm.Variable) {
            ...
            if (this.manageLiveVar) this._dataSetConnection = dojo.connect(this.dataSet, "onSuccess", this, "renderRows");

            if (this.dataSet.onBeforeUpdate) this.connect(this.dataSet, "onBeforeUpdate", this, function() {
                this.setLoadingImageShowing(true);
            });

            for (var i = 0; i < this.rowRenderers.length; i++) {
                if (this.rowRenderers[i].variable.type != inDataSet.type) this.rowRenderers[i].variable.setType(inDataSet.type);
            }
            ...
            if (!this._cupdating) this.renderRows();
        }
    },
    destroy: function() {
    ...
    renderRows: function() {
        ...
        var length;
        if (this.manageLiveVar) length = this.dataSet.dataSetCount - 1; // fix to this also in setDataSet
        else length = this.dataSet.getCount();
        ...
        for (var i = 0; i < length; i++) {
            ...
            if (!this.currentRenderer) {
                ...
                this.currentRenderer = this[name] = this.rowRenderers[i] = dojo.mixin(new wm.ListViewerRow({
                    name: name,
                    owner: this,
                    parent: this,
                    replacementNode: this.nodes[i]
                }), this._js);
            ...
            if (!this.renderVisibleRowsOnly || this.isScrolledIntoView(heightSum, this.currentRenderer.bounds.h, bounds)) {
                ...
                if (resetData) {
                    ...
                    if (!this.currentRenderer._noData) this.onRerenderRow(this.currentRenderer, this.currentRenderer.variable);
                } else {
    ...
    getTargetItem: function(inSender) {
        ...
        if (index >= 0) return this.dataSet.getItem(index);
    },
    setRowBorder: function(inBorder) {
        this.rowBorder = inBorder;
        dojo.forEach(this.rowRenderers, function(r) {
